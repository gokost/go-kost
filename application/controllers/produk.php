...
class Produk extends CI_Controller {
	...
	{
		parent::__construct();
		//$this->_cek_login(); // iki global, semua function di halaman ini jika perintah ini tidak di komentar maka semua membutuhkan login.
	...
	public function index()
	{
		$data = array(
			...
			'data_produk' => $this->model->GetProdukKatlokasio("order by id_produk desc")->result_array(),
		);
	...
	function addproduk()
	{
		$data = array(
			...
			'optlokasi' => $this->model->GetLokasi()->result_array(),
		);
	...
	function savedaftar(){
		...
		$result = $this->model->Simpan('tb_produk', $data);
		if($result == 1){
			$this->session->set_flashdata("sukses", "<div class='alert alert-warning'><strong>Simpan data BERHASIL dilakukan</strong></div>");
			header('location:'.base_url().'produk/sukses');
		}else{
			$this->session->set_flashdata("alert", "<div class='alert alert-danger'><strong>Simpan data GAGAL di lakukan</strong></div>");
			header('location:'.base_url().'produk/adddaftar');
		}		
	}
	...
	function editproduk($kode = 0){
		...
		$lokasi_post_array = array();
		foreach($this->model->GetProduk("where id_produk = '$kode'")->result_array() as $lokasi){
			$lokasi_post_array[] = $lokasi['id_lokasi'];
		}
		...
		$data = array(
			...
			'lokasi' => $this->model->GetLokasi()->result_array(),
			'label_post' => $kategori_post_array,
			'lokasi_post' => $lokasi_post_array,
			);
...
// Developed by Go-Kost
// Email: info@go-kost.com
