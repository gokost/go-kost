...
class Site extends CI_Controller {
	...
	function sidebar_kat(){
		$data = array(
			...
			"total_lokasi" => $this->model->GetProduk('group by id_lokasi')->num_rows(),
	...
	function galeri($id_produk = '', $kode = 0, $offset=0)
	{
		...
		/* Pagination */
		$config['uri_segment'] = 3;
		$config['base_url'] = base_url().'site/galeri';
		$config['total_rows'] = $this->model->count_all();
		$config['per_page'] = 6;
		...
		//$this->cookiesetter($kode);
		$this->load->view('site/galeri', $datas);
	}
	...
	function lokasi($id, $offset=0)
	{
		$this->countervisitor();
		$config['uri_segment'] = 4;
		$config['base_url'] = base_url().'site/lokasi/'.$id;
		$config['total_rows'] = $this->model->count_all();
		$config['per_page'] = 6;

		$cek = $this->model->GetLokasi("where id_lokasi = '$id'");
		if ($cek->num_rows() > 0) {
			$data = array(
				"data_produk" => $this->model->GetProduk("where tb_produk.status = 'publish' and id_lokasi = '$id' limit ".$config['per_page']." offset ".$offset)->result_array(),
				"rekomen" => $this->model->GetProduk("where status = 'publish' order by rand() limit 3")->result_array(),
				);
		 ...
		 // CSS Bootstrap               
			...
			$this->load->view('site/lokasi', $datas);
		}
	...
	private function cookiesetter($kode = 0){
		if(!empty($content[0])){
			...
			if($result == 1){
				setcookie($kode,"http://gokost.pe.hu", time()+3600);
			}
		}
	}
